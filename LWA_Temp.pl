	...
	if (unpack('H*',$saw) eq unpack('H*',$initialReq)) {
		...
		if (unpack('H*',$saw) eq unpack('H*',$initialRepl)) {
			while ($timeout>0) {
			  $port->purge_all;
			  ################################################ Get Sensors from DB ######################################
			  my $res = $dbh->selectall_arrayref("SELECT ID, Address, Name FROM sensors where Type='LWZ';");
			  ################################################ Request Values from LWZ ##################################				
			  foreach my $row (@$res) {
			    $SIG{'INT'} = 'IGNORE'; # Prevent from CTRL-C to allow a complete fetch of all sensors
  			    my ($id, $address) = @$row;
				#check of address has valid values		
				  ...
				  if (unpack('H*',$saw) eq unpack('H*',$Req)) {
				     ...
				     my $value;
				     if ($first == 0) {$value = $second} elsif ($second == 0) {$value = $first} else {$value=0};
				     
				     print "Insert $value for sensor ${$row}[2] with id ${$row}[0]\n" if $debug;
				...
				$SIG{'INT'} = 'INT_handler'; #Allow CTRL-C and run function INT_handler to shutdown DB connection
				print "===============================================================================\n" if $debug;
				sleep $STALL_DEFAULT;
				###########################################################################################################
		     ...
		     $SIG{'INT'} = 'INT_handler'; #Allow CTRL-C and run function INT_handler to shutdown DB connection
		     $timeout--;
		     print "Not connected to LWA. Timeout in $timeout seconds\n";
		}	
	}
...
# This function handles a CTRL-C to gracefully shutdown DB connection
sub INT_handler {
  print "CTRL-C Pressed; Closing DB connection\n";
  $dbh->disconnect();
  exit(0);
}
