...
$result = $db->query("select S_Value,strftime('%H:%M', Date_Time) from S_Values where Date_Time>=DATETIME('NOW','localtime','-12 hours') and Sensor_ID=1");
$result1 = $db->query("select S_Value,strftime('%H:%M', Date_Time) from S_Values where Date_Time>=DATETIME('NOW','localtime','-12 hours') and Sensor_ID=10");
...
<canvas id="cvs" width="1000" height="400" style="position:absolute;top:100px;left:20px;">[No canvas support]</canvas>
	...
	//chart.Set('chart.title', 'Au\u00dfentemperatur und Vorlauf Ist');
	

	// Set the legend
	chart.Set('chart.key', ['Au\u00dfentemperatur','Vorlauf Ist']);
