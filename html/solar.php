...
$result = $db->query("select S_Value,strftime('%H:%M', Date_Time) from S_Values where Date_Time>=DATETIME('NOW','localtime','-12 hours') and Sensor_ID=7");
$result1 = $db->query("select S_Value,strftime('%H:%M', Date_Time) from S_Values where Date_Time>=DATETIME('NOW','localtime','-12 hours') and Sensor_ID=2");
...
<canvas id="cvs" width="900" height="400" style="position:absolute;top:100px;left:20px;">[No canvas support]</canvas>
    <script>
	...
	// Set the legend
	chart.Set('chart.key', ['Speicher unten','Solar Panel']);
